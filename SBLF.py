...
def Acquire_Sheets():
    ...
    global Amount_People, List_Cat, Sub_Tags_Cat
...
def Create_Window():
    ...
    global my_objects, Variable_CB_0, Sub_Tags_Cat
    ...
    lista=[1,2,3]
    my_objects = []
    for o in range(len(Sub_List_Variables)):
        k = 0
        Row = 0
        Column = 0
        labelframe.append(LabelFrame(SW, text=Sub_Tags_Cat[o]))
        labelframe[o].grid(sticky=W+E,row=o, column=0)
        frame.append(Frame(labelframe[o], bd=1))
        
        
        sub_my_objects = []
        for i in range(len(Sub_List_Variables[o])):
            sub_my_objects.append(object)
            Var.append(IntVar())
        
        Vare = IntVar()
        for i in range(len(Sub_List_Variables[o])):
            Var_G   = IntVar()
            sub_my_objects[k]    = Checkbutton(frame[o], text=Sub_List_Variables[o][k], variable=Var_G)
            sub_my_objects[k].grid(sticky=W, row=Row, column=Column)
            Column  = Column + 1
            if Column > 5:
                Row     = Row + 1
                Column  = 0
            ...
            k = k + 1
        frame[o].grid(sticky=W+E,row=o, column=0)
        my_objects.append(sub_my_objects)
    ...
    labelframe2.grid(sticky=W+E,row=1+o, column=0)
    ...
    labelframe3.grid(sticky=W+E,row=2+o, column=0)
    ...
    labelframe4.grid(sticky=W,row=3+o, column=0)
    frame4 = Frame(labelframe4, bd=1)
    
    my_objects_3 = []
    for i in range(len(Sub_List_Variables)):
        my_objects_3.append(object)

    Var_G4   = IntVar()
    for i in range(len(Sub_List_Variables)):
        my_objects_3[k]    = Radiobutton(frame4, text=Sub_Tags_Cat[i], variable=Var_G4, value=k)
        my_objects_3[k].grid(sticky=W, row=Row, column=Column)
        Column  = Column + 1
        if Column > 5:
            Row     = Row + 1
            Column  = 0
    ...
    B_0 = Tkinter.Button(frame4, text = "Toggle Variable"    , command = lambda: Toggle(Var_G4.get()))
    B_1 = Tkinter.Button(frame4, text = "Leyenda"   , command = Leyenda)
    B_2 = Tkinter.Button(frame4, text = "Cerrar"    , command = Close)
    

    for i in [B_0,B_1,B_2]:
...
def Toggle(k):
    ...
    for i in range(len(Sub_List_Variables[k])):
        my_objects[k][i].toggle()
    
def Toggle_00():
    global my_objects, Sub_List_Variables
    for i in range(len(Sub_List_Variables[0])):
        my_objects[0][i].toggle()

def Toggle_01():
    global my_objects, Sub_List_Variables
    for i in range(len(Sub_List_Variables[1])):
        my_objects[1][i].toggle()
        
def Toggle_02():
    global my_objects, Sub_List_Variables
    for i in range(len(Sub_List_Variables[2])):
        my_objects[2][i].toggle()
